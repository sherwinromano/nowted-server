...
// ?? Get all notes
router.get("/", (req, res) => {
  const stmt = db.prepare("SELECT * FROM personal");
...
router.get("/:id", (req, res) => {
  const { id } = req.params;
  const stmt = db.prepare("SELECT * FROM personal WHERE id = ?");
...
router.patch("/:id", (req, res) => {
  ...
  const stmt = db.prepare(
    "UPDATE personal SET title = ?, date = ?, description = ? WHERE id = ?"
  );
...
router.post("/favorites/:id", (req, res) => {
  ...
  const getStmt = db.prepare("SELECT * FROM personal WHERE id = ?");
  ...
  const deleteStmt = db.prepare("DELETE FROM personal WHERE id = ?");
  deleteStmt.run(id);
...
router.delete("/trash/:id", (req, res) => {
  ...
  insertStmt.run(record.title, record.date, record.description, "personal");
