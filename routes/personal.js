...
router.get("/", (req, res) => {
  ...
  const notesStmt = db.prepare(
    "SELECT * FROM personal WHERE email = ? ORDER BY position ASC"
  );
...
router.patch("/reorder", (req, res) => {
  ...
  try {
    const updateStmt = db.prepare(
      "UPDATE personal SET position = ? WHERE id = ?"
    );
...
router.get("/:id", (req, res) => {
  ...
  const stmt = db.prepare("SELECT * FROM personal WHERE id = ? AND email = ?");
...
router.post("/", (req, res) => {
  ...
  const stmt = db.prepare(
    "INSERT INTO personal (email, title, date, description) VALUES (?, ?, ?, ?)"
  );
...
router.patch("/:id", (req, res) => {
  ...
  const stmt = db.prepare(
    "UPDATE personal SET description = ? WHERE id = ? AND email = ?"
  );
...
router.post("/favorites/:id", (req, res) => {
  ...
  const getStmt = db.prepare(
    "SELECT * FROM personal WHERE id = ? AND email = ?"
  );
  ...
  const deleteStmt = db.prepare(
    "DELETE FROM personal WHERE id = ? AND email = ?"
  );
